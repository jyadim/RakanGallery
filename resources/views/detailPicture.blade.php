                ...
                <div class="grid gap-4 bg-white rounded-lg shadow-lg p-4">
                    ...
                    <div class="flex space-x-6">
                        ...
                        <button
                            ...
                            class="py-2 px-4 w-28 bg-yellow-500 text-white rounded-lg">
                            Edit
                        </button>
                        ...
                        <form action="{{ route('photo.destroy', $pict->id) }}" method="POST"
                            ...
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="py-2 px-4 w-28 bg-red-600 text-white rounded-lg">
                                Delete
                            </button>
                        </form>
                    </div>
                    ...
                    <!-- Update Form Modal -->
                    <div id="updateForm-{{ $pict->id }}"
                        class="hidden mt-4 p-6 bg-white rounded-lg shadow-md w-full max-w-lg mx-auto">
                        <form action="{{ route('photo.update', $pict->id) }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="photo_title" value="{{ $pict->photo_name }}"
                                    class="w-full p-4 border border-indigo-600 rounded-lg" required>
                            </div>
                            <div class="mb-4">
                                <label class="block text-gray-700">Description</label>
                                <input type="text" name="desc" value="{{ $pict->photo_desc }}"
                                    class="w-full p-4 border border-indigo-600 rounded-lg">
                            </div>
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <button type="button" onclick="closeUpdateForm('{{ $pict->id }}')"
                                    class="py-2 px-6 bg-gray-300 text-gray-700 rounded-lg">Cancel</button>
                            </div>
                        </form>
                    </div>
