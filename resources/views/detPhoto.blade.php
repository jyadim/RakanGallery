                        ...
                        <div class="flex items-center space-x-4  mb-6">
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <form action="{{ route('photo.like', $photo->id) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="flex items-center {{ auth()->user() && $photo->like->where('user_id', auth()->id())->count() ? 'text-red-500' : 'text-gray-700' }} hover:text-red-500">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            ...
                                            6.86-8.55 11.54L12 21.35z"
                                                @if (auth()->user() && $photo->like->where('user_id', auth()->id())->count()) fill="currentColor" @endif />
                                        </svg>
                                        <span class="ml-1 text-sm font-medium">{{ $photo->like->count() }}</span>
                                    </button>
                                </form>
